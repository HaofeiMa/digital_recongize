...
void MyDemo::gamma_Demo(Mat& image) {
	/*
	Mat gray_image, gamma_image;
	...
	*/
	...
	cv::minMaxLoc(image, &pixMin, &pixMax);
...
void MyDemo::bit_Demo(Mat& image) {
	//获取BGR三通道最大值
	int w = image.cols;
	int h = image.rows;
	...
	unsigned char DifMax = 25;             //基于颜色区分的阈值设置
	unsigned char WhiteMax = 50;		   //判断白色
	//unsigned char B = 200, G = 200, R = 200; //各通道的阈值设定，针对与A4纸
	int i = 0, j = 0;
	for (i = 0; i < image.rows; i++)   //通过颜色分量将图片进行二值化处理
	{
		for (j = 0; j < image.cols; j++)
		{
			...
			if (((bmax - pixelB) < DifMax) && ((gmax - pixelG) < DifMax) && ((rmax - pixelR) < DifMax) && abs(pixelB - pixelG) < WhiteMax && abs(pixelG - pixelR) < WhiteMax && abs(pixelB - pixelR) < WhiteMax)
			{                                           //将各个通道的值和各个通道阈值进行比较
